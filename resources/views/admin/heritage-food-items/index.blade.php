    ...
    <section class="panel food-add-panel">
        ...
        <form method="POST" action="{{ route('admin.heritage-shops.food-items.store', $shop) }}" enctype="multipart/form-data">
            ...
            <div class="food-form-grid">
                ...
                <div class="field"><label for="new-image">Food photo</label><input id="new-image" name="image" type="file" accept="image/jpeg,image/png,image/webp" data-heritage-image-input data-max-bytes="{{ config('heritage_shop.max_image_bytes', 2097152) }}"><span class="help-text">JPG, PNG, or WebP · max {{ number_format(config('heritage_shop.max_image_kb', 2048) / 1024, 0) }} MB</span><span class="image-size-error status-banner error" data-image-error style="display:none; margin:0; padding:8px 10px;"></span></div>
                    ...
                    <article class="food-item-admin-card">
                        ...
                        <form method="POST" action="{{ route('admin.heritage-shops.food-items.update', [$shop, $item]) }}" enctype="multipart/form-data">
                            ...
                            <div class="food-form-grid compact">
                                ...
                                <div class="field"><label for="image-{{ $item->id }}">Replace photo</label><input id="image-{{ $item->id }}" name="image" type="file" accept="image/jpeg,image/png,image/webp" data-heritage-image-input data-max-bytes="{{ config('heritage_shop.max_image_bytes', 2097152) }}"><span class="help-text">Leave empty to keep the current photo. New photo max {{ number_format(config('heritage_shop.max_image_kb', 2048) / 1024, 0) }} MB.</span><span class="image-size-error status-banner error" data-image-error style="display:none; margin:0; padding:8px 10px;"></span></div>
...
<script>
(() => {
    const maxLabel = '{{ number_format(config('heritage_shop.max_image_kb', 2048) / 1024, 0) }} MB';
    document.querySelectorAll('[data-heritage-image-input]').forEach((input) => {
        input.addEventListener('change', () => {
            ...
            if (file && file.size > Number(input.dataset.maxBytes || 2097152)) {
                if (error) {

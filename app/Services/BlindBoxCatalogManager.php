...
class BlindBoxCatalogManager
{
    ...
    // Blind Box. These samples keep the public recommendation page useful
    // before admins customize the pool.
    private const DEFAULT_INCLUDED_SOURCE_IDS = [
        'kampung kuih mak cik',
        'kedai kopi pak hassan',
        'nasi lemak seri warisan',
    ];
        ...
        $pools = Cache::rememberForever(self::CACHE_KEY, function () use ($sourceByKey): array {
            return $sourceByKey
                ...
                ->map(fn (array $shop): array => array_merge($shop, [
                    'id' => $shop['source_id'],
                    'category' => $shop['category'] ?? 'Main Dishes',
                ]))
                ->values()
                ->all();
        });
